...
//                   <select name="jobType" multiple {...register('jobType')} 
//                   className='input'>
//                     <option value="">Select</option>
//                     <option value="Full Time">Full Time</option>
//                     <option value="Part Time">Part Time</option>
...
const salaryTypes = [
  { id: 'hourly', value: 'Hourly', label: 'Hourly' },
...
const FindJobs = () => {
  ...
  const watchedValues = watch();
  const selectedLicenses = useWatch({ control, name: 'selectedLicenses', defaultValue: [] });
  const selectedJobTypes = useWatch({ control, name: 'selectedJobTypes', defaultValue: [] });
  const selectedSalaryTypes = useWatch({ control, name: 'selectedSalaryTypes', defaultValue: [] });
  const selectedJobLocations = useWatch({ control, name: 'selectedJobLocations', defaultValue: [] });
  ...
  const handleSubmitForm = async (data) => {
    ...
    const filters = {
      ...
      salaryType: selectedSalaryTypes,
      requiredSkills: data.requiredSkills,
      jobLocation: selectedJobLocations,
      publishedDate: data.publishedDate,
      passport: data.passport,
      visa: data.visa,
      abroadExperience: data.abroadExperience,
       ...
       <div className='flex gap-1'>
         ...
         <div className='w-[33%]'>
               ...
               {jobTypes.map((jobType) => (
                 <div key={jobType.id} className="flex items-center">
                   <input
                     type="checkbox"
                     id={jobType.id}
                     {...register("selectedJobTypes")}
                     checked={selectedJobTypes.includes(jobType.value)}
                     onChange={(e) => handleCheckboxChange(e, "selectedJobTypes")}
                   />
                   <label htmlFor={jobType.id} className="ml-2">
                     {jobType.label}
                   </label>
                 </div>
               ))}
           ...
           <div className="flex flex-col gap-1 lg:w-[50%]">
             ...
             <div className="flex flex-wrap gap-2">
               {salaryTypes.map((salaryType) => (
                 <div key={salaryType.id} className="flex items-center">
                   <input
                     type="checkbox"
                     id={salaryType.id}
                     {...register("selectedSalaryTypes")}
                     checked={selectedSalaryTypes.includes(salaryType.value)}
                     onChange={(e) => handleCheckboxChange(e, "selectedSalaryTypes")}
                   />
                   <label htmlFor={salaryType.id} className="ml-2">
                     {salaryType.label}
                   </label>
                 </div>
               ))}
           ...
           <div className="flex flex-col gap-1 lg:w-[50%]">
             ...
             <div className="flex flex-wrap gap-2">
               {states.map((state) => (
                 <div key={state.isoCode} className="flex items-center">
                   <input
                     type="checkbox"
                     id={state.name}
                     {...register("selectedJobLocations")}
                     checked={selectedJobLocations.includes(state.name)}
                     onChange={(e) => handleCheckboxChange(e, "selectedJobLocations")}
                   />
                   <label htmlFor={state.name} className="ml-2">
                     {state.name}
                   </label>
                 </div>
               ))}
         ...
         <div className='w-[33%]'>
           <div className="relative w-full" ref={dropdownRef}>
             <label htmlFor="selectedLicenses">Select Licenses</label>
             <button
               type="button"
               className="input-style"
               onClick={toggleDropdown}
             >
               {selectedLicenses.length > 0 ? `${selectedLicenses.length} license(s) selected` : 'Select Licenses'}
             </button>
             {dropdownOpen && (
                 ...
                 {licenseCategories.map((category) => (
                   <div key={category.id} className="flex items-center p-2">
                     <input
                       ...
                       checked={selectedLicenses.includes(category.value)}
                       onChange={handleLicenseCheckboxChange}
                       className="mr-2"
                     />
           ...
           <div className="flex flex-col gap-1">
             <div className="flex items-center">
               <input type="checkbox" id="passport" {...register("passport")} className="mr-2" />
               <label htmlFor="passport">Passport</label>
             </div>
             <div className="flex items-center">
               <input type="checkbox" id="visa" {...register("visa")} className="mr-2" />
               <label htmlFor="visa">Visa</label>
             </div>
             <div className="flex items-center">
               <input type="checkbox" id="abroadExperience" {...register("abroadExperience")} className="mr-2" />
               <label htmlFor="abroadExperience">Abroad Experience</label>
             </div>
             <div className="flex items-center">
               <input type="checkbox" id="isBlindSpotTraining" {...register("isBlindSpotTraining")} className="mr-2" />
               <label htmlFor="isBlindSpotTraining">Blind Spot Training</label>
             </div>
             <div className="flex items-center">
               <input type="checkbox" id="isSafeDrivingTraining" {...register("isSafeDrivingTraining")} className="mr-2" />
               <label htmlFor="isSafeDrivingTraining">Safe Driving Training</label>
             </div>
             <div className="flex items-center">
               <input type="checkbox" id="isFuelEconomyTraining" {...register("isFuelEconomyTraining")} className="mr-2" />
               <label htmlFor="isFuelEconomyTraining">Fuel Economy Training</label>
             </div>
           </div>
         </div>
        ...
        <div className="w-full flex justify-between">
          <button type="submit" className='primary-button'>
            {fetchingJobs ? 'Searching...' : 'Search Jobs'}
          </button>
        </div>
